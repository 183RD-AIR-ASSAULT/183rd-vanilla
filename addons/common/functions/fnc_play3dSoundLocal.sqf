 ...
 * \OETAV_common\functions\sounds\fn_play3dSoundLocal.sqf
 * by Ojemineh
 *
 ...
 *
 * Example:
 * [player, "AlarmBell", [], -1, -1, 6] call OETAV_common_fnc_play3dSoundLocal;
 *
 */
...
if (_distance <= 0) then {_distance = [_soundClass] call OETAV_common_fnc_getSoundDistance;};
if (_distance <= 0) then {_distance = 100;};
//if ((_maxDistance > 0) && ((player distance _object) > _maxDistance)) exitWith {};

if (_duration <= 0) then {_duration = [_soundClass] call OETAV_common_fnc_getSoundDuration;};
if (_duration <= 0) exitWith {};

if (_pitch <= 0) then {_pitch = [_soundClass] call OETAV_common_fnc_getSoundPitch;};
if (_pitch <= 0) then {_pitch = 1;};

// -------------------------------------------------------------------------------------------------

private _soundArray = _object getVariable ["OETAV_sound3d", []];
...
if (_soundInUse) exitWith {
    [_object, _soundClass, _position, _distance, _maxDistance, _duration, _pitch, _isSpeech] spawn {
        ...
        [_object, _soundClass] call OETAV_common_fnc_stop3dSoundLocal;
        sleep 0.1;
        [_object, _soundClass, _position, _distance, _maxDistance, _duration, _pitch, _isSpeech] call OETAV_common_fnc_play3dSoundLocal;
    };
};

// -------------------------------------------------------------------------------------------------

private _soundObject = createSimpleObject [OETAV_COMMON_HELPER_OBJECT, [0,0,0], true];
_soundObject hideObject true;
_soundObject attachTo [_object, _position];

_soundArray pushBackUnique [_soundClass, _soundObject];
_object setVariable ["OETAV_sound3d", _soundArray];
...
private _handler = [_object, _soundObject, _soundClass, _distance, _maxDistance, _duration, _pitch, _isSpeech] spawn {
    ...
    [_soundObject, _soundClass, _distance, _maxDistance, _pitch, _isSpeech] call OETAV_common_fnc_say3dLocal;
