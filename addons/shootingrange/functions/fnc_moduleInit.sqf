...
private _targets = [_logic getVariable "Targets", true, false] call ACEFUNC(core,parseList);
...
private _targetsInvalid = [_logic getVariable "TargetsInvalid", true, false] call ACEFUNC(core,parseList);
...
private _controllers = [_logic getVariable "Controllers", true, true] call ACEFUNC(core,parseList);

// Exctract sound source objects
private _soundSources = [_logic getVariable "SoundSources", true, true] call ACEFUNC(core,parseList);

// Extract target change event
private _mode = _logic getVariable "Mode";

// Extract duration string and convert to numbers
private _durations = ([_logic getVariable "Durations", true, false] call ACEFUNC(core,parseList) apply {parseNumber _x});
...
private _targetAmounts = ([_logic getVariable "TargetAmounts", true, false] call ACEFUNC(core,parseList) apply {parseNumber _x});
...
private _pauseDurations = ([_logic getVariable "PauseDurations", true, false] call ACEFUNC(core,parseList) apply {parseNumber _x});
...
private _countdownTimes = ([_logic getVariable "CountdownTimes", true, false] call ACEFUNC(core,parseList) apply {parseNumber _x});

// Extract default countdown time
private _defaultCountdownTime = _logic getVariable "DefaultCountdownTime";

// Exctract triggers
private _triggerMarkers = [_logic getVariable "TriggerMarkers", true, false] call ACEFUNC(core,parseList);
