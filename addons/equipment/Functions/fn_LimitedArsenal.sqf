/*
Function: OETAV_fnc_LimitedArsenal
...
Description:
 ...
 OETAV_fnc_Arsenal, only the items in the container change.
...
Examples:
 (begin example)
  [this] call OETAV_fnc_LimitedArsenal;
  [this, "Aegis", 50] call OETAV_fnc_LimitedArsenal;
  [this, "VANILLA"] call OETAV_fnc_LimitedArsenal;
  [MyAmmoCrate, "RHS", 0] call OETAV_fnc_LimitedArsenal;
 (end)
...
call {
 if (_Type isEqualto 0) exitwith {_Type = "OETAV"};
...
call {
 if (_Type isEqualTo "OETAV") exitwith {
  _AvailableItems =[];
        _AvailableHeadgear = [];
        _AvailableGoggles = [];
        _AvailableUniforms = [];
